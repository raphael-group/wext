...
def saddlepoint_exclusive_4(z, m_u, m_w, m_x, m_y, p_u, p_w, p_x, p_y, verbose=False):
    ...
    q_x = 1.0-p_x
    q_y = 1.0-p_y
    m = np.array([m_u, m_w, m_x, m_y, z-0.5])

    # Define K.

    def L(p, q, r, s, t):
        return np.exp(p+t)*p_u*q_w*q_x*q_y + np.exp(q+t)*q_u*p_w*q_x*q_y + np.exp(r+t)*q_u*q_w*p_x*q_y + np.exp(s+t)*q_u*q_w*q_x*p_y + \
               np.exp(p+q)*p_u*p_w*q_x*q_y + np.exp(p+r)*p_u*q_w*p_x*q_y + np.exp(p+s)*p_u*q_w*q_x*p_y + \
               np.exp(q+r)*q_u*p_w*p_x*q_y + np.exp(q+s)*q_u*p_w*q_x*p_y + np.exp(r+s)*q_u*q_w*p_x*p_y + \
               np.exp(p+q+r)*p_u*p_w*p_x*q_y + np.exp(p+q+s)*p_u*p_w*q_x*p_y + np.exp(p+r+s)*p_u*q_w*p_x*p_y + np.exp(q+r+s)*q_u*p_w*p_x*p_y + \
               np.exp(p+q+r+s)*p_u*p_w*p_x*p_y + q_u*q_w*q_x*q_y
...
#        return np.array([K_q(q, r, s, t), K_r(q, r, s, t), K_s(q, r, s, t), K_t(q, r, s, t)])
    ...
    def dK_(p, q, r, s, t):
        ...
        x = [np.exp(p+t)*p_u*q_w*q_x*q_y,
             np.exp(q+t)*q_u*p_w*q_x*q_y,
             np.exp(r+t)*q_u*q_w*p_x*q_y,
             np.exp(s+t)*q_u*q_w*q_x*p_y,
             np.exp(p+q)*p_u*p_w*q_x*q_y,
             np.exp(p+r)*p_u*q_w*p_x*q_y,
             np.exp(p+s)*p_u*q_w*q_x*p_y,
             np.exp(q+r)*q_u*p_w*p_x*q_y,
             np.exp(q+s)*q_u*p_w*q_x*p_y,
             np.exp(r+s)*q_u*q_w*p_x*p_y,
             np.exp(p+q+r)*p_u*p_w*p_x*q_y,
             np.exp(p+q+s)*p_u*p_w*q_x*p_y,
             np.exp(p+r+s)*p_u*q_w*p_x*p_y,
             np.exp(q+r+s)*q_u*p_w*p_x*p_y,
             np.exp(p+q+r+s)*p_u*p_w*p_x*p_y,
             q_u*q_w*q_x*q_y]
